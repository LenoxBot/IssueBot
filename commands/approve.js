...
exports.run = async (client, msg, args) => {
	...
	if (msg.channel.id === settings.processingBugreportsChannel) {
		...
		if (args.slice(1).join(' ').length > 100) return msg.delete() && msg.reply('Your approve text has to have <100 characters!').then(m => m.delete(10000));
		...
		collector.on('end', async (collected, reason) => {
			...
			if (Object.keys(issueconfs.approve).length >= 1) {
				msg.guild.members.get(issueconfs.authorid).send(`Thank you for your bugreport! Your bugreport has been accepted and will be processed as soon as possible. As a thank, you got 200 credits! \n\n\`\`\`${fetchedmessage.embeds[0]}\`\`\``);
				...
				newEmbed.setURL(createdIssue.url);
